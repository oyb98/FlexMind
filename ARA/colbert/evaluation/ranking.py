...
def evaluate(args):
    ...
    metrics = Metrics(mrr_depths={10, 100}, recall_depths={5, 10, 50, 200, 1000},
    ...
    data = {"query": [], "gold documents": [], "retreive documents": [], "gold index": []}
    
    with open("./data/MSMARCO/q2kw_test.json", "r") as f:
        qid_keyword_dict = json.load(f)
    ...
    with ranking_logger.context('ranking.tsv', also_save_annotations=(qrels is not None)) as rlogger:
        with torch.no_grad():
            ...
            for query_idx, qid in enumerate(keys):
                ...
                keyword = qid_keyword_dict[query]
                ...
                ranking = slow_rerank(args, query, topK_pids[qid], qid2passages(qid), keyword)
                ...
                if qrels:
                    ...
                    data["retreive documents"].append(ranking[0][2])
                    flag = True
                    gold_index = 1001
                    for i, (score, pid, passage) in enumerate(ranking):
                        if pid in qrels[qid]:
                            ...
                            print("query: ", query)
                            data["gold documents"].append(passage)
                            flag = False
                            gold_index = i
                            break
                    if flag: data["gold documents"].append("")
                    data["gold index"].append(gold_index)
