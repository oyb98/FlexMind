...
def train(args):
    ...
    labels_DQQ = torch.zeros(args.bsize, dtype=torch.long, device=DEVICE)

    start_time = time.time()
    train_loss = 0.0
    train_col_loss = 0.0
    train_DQQ_col_loss = 0.0
    train_kw_loss = 0.0
    ...
    for batch_idx, BatchSteps in zip(range(start_batch_idx, args.maxsteps), reader):
        ...
        for queries, passages, kw_weight in BatchSteps:
            with amp.context():
                scores, kw_scores = colbert(queries, passages, keyword=kw_weight, trianing=True)
                scores = scores.view(3, -1).permute(1, 0)
                col_loss = criterion(scores[:, :2], labels[:scores.size(0)])
                
                DQQ_col_loss = criterion(scores[:, [0, 2]], labels_DQQ[:scores.size(0)])
                
                kw_scores = kw_scores.view(3, -1).permute(1, 0)
                kw_input = F.log_softmax(scores[:, :2], dim=1)
                target = F.softmax(kw_scores[:, :2].detach(), dim=1)
                kw_loss = kw_cri(kw_input, target)
                
                loss = col_loss + DQQ_col_loss + kw_loss
            ...
            train_col_loss += col_loss.item()
            train_DQQ_col_loss += DQQ_col_loss.item()
            train_kw_loss += kw_loss.item()
        ...
        if args.rank < 1:
            ...
            avg_col_loss = train_col_loss / (batch_idx+1)
            avg_DQQ_col_loss = train_DQQ_col_loss / (batch_idx+1)
            avg_kw_loss = train_kw_loss / (batch_idx+1)
            ...
            print_message(batch_idx, avg_loss, avg_col_loss, avg_DQQ_col_loss, avg_kw_loss)
